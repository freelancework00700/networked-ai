...
import { EventDetails } from '@/pages/create-event/components/event-details';
import { EventTickets } from '@/pages/create-event/components/event-tickets';
import { EventSettings } from '@/pages/create-event/components/event-settings';
import { EventForm, Ticket, PromoCode, SubscriptionPlan } from '@/interfaces/event';
import { Input, signal, inject, computed, Component, ChangeDetectionStrategy } from '@angular/core';
...
export class CreateEvent {
  ...
  constructor() {
    ...
    form.get('medias')?.valueChanges.subscribe(() => {
      this.formChangeTrigger.update((v) => v + 1);
    });
  }
